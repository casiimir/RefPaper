...
function detectDocumentationType(content: string): string {
  const patterns = [
    { type: 'docusaurus', indicators: ['__docusaurus', 'On this page', 'Skip to main content'] },
    { type: 'gitbook', indicators: ['Was this helpful?', 'Last updated', 'GitBook'] },
    { type: 'sphinx', indicators: ['Built with Sphinx', '© Copyright', 'sphinx-build'] },
    { type: 'vitepress', indicators: ['Edit this page on GitHub', 'VitePress'] },
...
function cleanMarkdown(content: string): string {
  ...
  let cleaned = content
    ...
    .replace(/We use.*?cookies[\s\S]*?(?:privacy|policy|learn more)[\s\S]*?\./gi, "")
  ...
  switch (docType) {
    ...
    case 'mkdocs':
      cleaned = cleaned
        .replace(/Created with Material for MkDocs/gi, "");
      break;
  }
