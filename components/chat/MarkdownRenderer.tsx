...
export const MarkdownRenderer = memo(function MarkdownRenderer({ content }: MarkdownRendererProps) {
  ...
  const { syntaxTheme, textStyles, codeStyles, codeBlockStyles } = useMemo(() => {
    const isDarkTheme =
      theme === "dark" ||
      (theme === "system" &&
    ...
    return {
      ...
      codeBlockStyles: {
        ...
        fontSize: "0.9rem",
        lineHeight: "1.5",
        backgroundColor: isDarkTheme ? "rgb(30, 30, 30)" : "rgb(248, 248, 248)",
      },
    };
  }, [theme]);
  ...
  const SafeCodeBlock = useMemo(
    () =>
      function SafeCodeBlock({ code, language }: { code: string; language: string }) {
        try {
          return (
            <Suspense fallback={<pre className="bg-muted p-4 rounded font-mono text-sm">{code}</pre>}>
              <SyntaxHighlighter
          ...
          return <pre className="bg-muted p-4 rounded font-mono text-sm">{code}</pre>;
        }
      },
  ...
  const renderContent = useMemo(() => (text: string) => {
  ...
  const processMarkdown = useMemo(() => (text: string, baseIndex: number) => {
    return text.split("\n").map((line, index) => {
  ...
  const processInlineCode = useMemo(() => (text: string) => {
  ...
  const renderedContent = useMemo(() => renderContent(content), [renderContent, content]);
